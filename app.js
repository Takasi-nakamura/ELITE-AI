...
const THINKING_LEVELS = {
  1: { label: "即答",     roman: "Ⅰ", thinkingLevel: "minimal", twoStage: false },
  2: { label: "軽い思考",  roman: "Ⅱ", thinkingLevel: "low",     twoStage: false },
  3: { label: "標準思考",  roman: "Ⅲ", thinkingLevel: "medium",  twoStage: false },
  4: { label: "深い思考",  roman: "Ⅳ", thinkingLevel: "high",    twoStage: true  },
  5: { label: "最深思考",  roman: "Ⅴ", thinkingLevel: "high",    twoStage: true  }
};
...
async function callGeminiOnce(model, contents, systemInstruction, thinkingLevel) {
  ...
  // Gemini 3.x系（3.5/3.6）では数値のthinkingBudgetは廃止され、
  // thinkingLevel（"minimal"/"low"/"medium"/"high"）を使う
  if (thinkingLevel) {
 ...
 */
async function callGeminiTwoStage(contents, systemInstruction, thinkingLevel) {
  const stage1System = systemInstruction + "\n\n【内部思考モード】これは最終回答ではありません。ユーザーの意図を深く分析し、複数の観点から検討した下書きを作成してください。";
  const draft = await callGeminiOnce("gemini-3.6-flash", contents, stage1System, thinkingLevel);
  ...
  const final = await callGeminiOnce("gemini-3.6-flash", stage2Contents, stage2System, thinkingLevel);
  return { draft, final };
}
...
async function sendMessage() {
  ...
  try {
    ...
    if (useTwoStage) {
      const result = await callGeminiTwoStage(contents, sys, levelInfo.thinkingLevel);
      finalText = result.final;
      draftText = result.draft;
    } else {
      finalText = await callGeminiOnce(modelUsed, contents, sys, levelInfo.thinkingLevel);
    }
